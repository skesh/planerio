  ...
  if (!todo) {
    return (
      <View className="flex-1 items-center justify-center" style={{ backgroundColor: colors.bg }}>
        <Text className="text-lg" style={{ color: colors.textMuted }}>Todo not found</Text>
      </View>
    )
  }
    ...
    <View className="flex-1" style={{ backgroundColor: colors.bg, paddingTop: top }}>
      <View className="flex-row items-center justify-between px-4 py-3 border-b" style={{ borderColor: colors.border }}>
        <Pressable onPress={() => router.back()} className="p-1">
          <Text className="text-lg" style={{ color: colors.textMuted }}>← Back</Text>
        </Pressable>
        <Text className="text-base font-semibold" style={{ color: colors.text }}>Edit task</Text>
